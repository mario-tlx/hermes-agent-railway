...
**Authenticator (TOTP):** Generate a Base32 secret (only letters A–Z and digits 2–7). Example: `python -c "import base64, os; print(base64.b32encode(os.urandom(10)).decode().rstrip('='))"`. Set it as `ADMIN_TOTP_SECRET` in Railway. On **first** successful login after setup, the `/totp` page shows a **QR** once; later logins only ask for the **6-digit code** (the QR is hidden so password-only attackers cannot enroll a new device). If you **rotate** `ADMIN_TOTP_SECRET`, the QR appears again until you verify once. Enrollment is stored in `{HERMES_HOME}/.admin_totp_state.json` on the volume (delete that file to force QR setup again for the same secret).
